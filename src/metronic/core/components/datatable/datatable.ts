...
import type { KTDataTableCheckChangePayloadInterface } from './types'
  ...
  implements KTDataTableInterface
{
  ...
  private _attachSearchEvent(): void {
    ...
    const searchElement: HTMLInputElement | null = document.querySelector<HTMLInputElement>(
      ...
      .then((response) => {
        if (!response.ok) {
          ...
          this._fireEvent('error', {
            error: new Error('Error performing fetch request: ' + response.statusText),
          })
  ...
  private _setPaginationInfoText(_infoElement: HTMLElement): void {
    _infoElement.textContent = this._config.info
      ...
      .replace(
        '{end}',
        Math.min(this.getState().page * this.getState().pageSize, this.getState().totalItems) + '',
      )
  ...
  private _showSpinner(): void {
    const spinner =
      this._element.querySelector<HTMLElement>(this._config.attributes.spinner) ||
      this._createSpinner()
    if (spinner) {
