...
import type { KTStickyInterface, KTStickyConfigInterface } from './types'
...
export class KTSticky extends KTComponent implements KTStickyInterface {
  ...
  constructor(element: HTMLElement, config: KTStickyConfigInterface | null = null) {
    ...
    const targetElement =
      this._getTarget() === 'body' ? document : KTDom.getElement(this._getTarget())
  ...
  private _getTarget(): string {
    return (
      (this._element.getAttribute('data-sticky-target') as string) ||
