    ...
    document.addEventListener('click', function (event: Event) {
      ...
      itemElements.forEach((itemElement) => {
        ...
        if (menu && menu.getItemToggleMode(itemElement as HTMLElement) === 'dropdown') {
          ...
          if (itemElement === event.target || itemElement.contains(event.target as HTMLElement)) {
            return
          }

          if (
            subElement &&
            (subElement === event.target || subElement.contains(event.target as HTMLElement))
          ) {
            return
          }
  ...
  public static getFocusLink(
    ...
    if (direction === 'next') {
      ...
      if (prevElement) {
        ...
        if (
          parentElement &&
          (parentElement.matches('.menu-accordion' + (!preFocus ? '.show' : '')) ||
            parentElement.matches('.menu-dropdown' + (!preFocus ? '.show' : '')))
        ) {
        ...
        if (
          ...
          if (event.key === 'Escape') {
            ...
            items.forEach((item) => {
              ...
              if (menu && menu.getItemToggleMode(item as HTMLElement) === 'dropdown') {
