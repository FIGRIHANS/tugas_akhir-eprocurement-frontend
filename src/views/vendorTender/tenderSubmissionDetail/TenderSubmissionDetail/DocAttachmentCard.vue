    ...
    <div class="card-body flex flex-col gap-[16px]">
      <div class="flex flex-col justify-between">
        ...
        <div class="file-status">
          <span v-if="!technicalFile" class="text-blue-500 cursor-pointer" @click="triggerFileUpload('technicalRef')">
            Upload Document
          </span>
          <div v-else class="flex items-center gap-2">
            ...
            <button @click="removeFile('technicalFile')" class="text-red-500 ml-auto">Delete</button>
          </div>
        </div>
        <input type="file" ref="technicalRef" @change="handleFileChange($event, 'technicalFile')" class="hidden" />
      </div>

      <div class="flex flex-col justify-between">
        <span class="text-gray-500">Tender Comercial Proposal</span>
        <div class="file-status">
          <span v-if="!commercialFile" class="text-blue-500 cursor-pointer" @click="triggerFileUpload('commercialRef')">
            Upload Document
          </span>
          <div v-else class="flex items-center gap-2">
            <!-- <img src="/path-to-pdf-icon.svg" alt="PDF Icon" class="w-5 h-5" /> -->
        ...
        <input type="file" ref="commercialRef" @change="handleFileChange($event, 'commercialFile')" class="hidden" />
      </div>

      <div class="flex flex-col justify-between">
        <span class="text-gray-500">Other Document</span>
        <div class="file-status">
          <span v-if="!otherFile" class="text-blue-500 cursor-pointer" @click="triggerFileUpload('otherRef')">
            Upload Document
          </span>
          <div v-else class="flex items-center gap-2">
            <!-- <img src="/path-to-pdf-icon.svg" alt="PDF Icon" class="w-5 h-5" /> -->
            <span class="text-gray-700">{{ otherFile.name }}</span>
            <button @click="removeFile('otherFile')" class="text-red-500 ml-auto">Delete</button>
          </div>
        </div>
        <input type="file" ref="otherRef" @change="handleFileChange($event, 'otherFile')" class="hidden" />
      </div>
    </div>
  </div>
</template>
