<template>
  <div class="bg-white">
    ...
    <div v-if="activeView === 'bpu'" class="animate-in fade-in duration-300">
      <div class="overflow-x-auto list__table rounded-xl overflow-hidden">
        <table class="table align-middle text-gray-700 font-medium text-sm">
          <thead>
            <tr>
...
const handleExportPdf = async (item: BpuContent) => {
  closeDropdowns()
  ...
  try {
    pdfBlobUrl.value = await generateBpuPdfBlobUrl(item)
  } catch (err) {
    console.error(err)
