<template>
        ...
        <div class="card-body">
              ...
              <div class="flex items-center gap-3">
                ...
                <div class="flex-1 max-w-[400px]">
                  <div class="relative flex items-center border border-gray-300 rounded-md bg-white">
                    <!-- Icon box -->
                    <div class="flex items-center justify-center w-10 h-10 bg-gray-300 border-r border-gray-300 rounded-l-md overflow-hidden">
                      <svg
          ...
          <div class="grid grid-cols-2 gap-6">
            ...
            <div class="p-4">
              <div class="flex items-center gap-3">
                <label class="text-sm font-medium text-gray-700 w-[140px] flex-shrink-0">
                  Erp Integration Config<span class="text-red-500">*</span>
                </label>
    ...
    <div v-if="isGenerated">
      ...
      <div v-for="(table, index) in tables" :key="index">
        <AddNewTable
          :profileGroupName="table.profileGroupName"
          :approveProfileId="table.approveProfileId"
          @add-approver="handleAddApprover"
          @add-notification="handleAddNotification"
          @cancel="() => handleCancel(index)"
          @back="() => handleBack(index)"
        ...
        <div class="card-body">
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider" v-for="item in integrationListColumns" :key="item.name">
                    {{ item.name }}
                </th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                <tr
                    v-for="item in integrationDummy.data"
                    :key="item.code"
                    class="hover:bg-gray-50"
                >   
                    <td class="px-3 py-2 text-sm font-medium text-gray-900 flex gap-2">
                        <button class="btn btn-outline btn-primary btn-icon w-[32px] h-[32px]"><i class="ki-filled ki-eye !text-lg"></i></button>
                        <button class="btn btn-outline btn-primary btn-icon w-[32px] h-[32px]"><i class="ki-duotone ki-data !text-lg"></i></button>
                    </td>
                    ...
                    <span
                        ...
                        :class="item.fieldMappingStatus === 'Active'
                        ? 'bg-green-100 text-green-700'
                        : 'bg-yellow-100 text-yellow-700'"
                    >
                        {{ item.fieldMappingStatus }}
                    </span>
                    </td>
                    ...
                    <span
                        class="inline-flex px-2 py-1 text-xs font-semibold rounded-full"
                        :class="item.integrationStatus === 'Active'
                        ? 'bg-green-100 text-green-700'
                    ...
                    <span
                        class="inline-flex px-2 py-1 text-xs font-semibold rounded-full"
                        :class="item.connectionTest === 'Success'
                        ? 'bg-green-100 text-green-700'
                        : 'bg-red-100 text-red-700'"
                    >
                        {{ item.connectionTest }}
  ...
  const integrationListColumns = ref([
    {
        name: 'Action'
    },
    {
