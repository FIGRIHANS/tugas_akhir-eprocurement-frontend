  ...
  <div>
    ...
    <UiModal v-model="showModalSuccess" size="sm">
      <div class="text-center mb-6">
        ...
        <p class="text-center text-base text-gray-600 mb-5">Employee successfully updated</p>
      </div>
    </UiModal>
  </div>
</template>
...
const { handleSubmit, meta, errors, isSubmitting, setFieldValue, resetForm } = useForm({
...
onMounted(async () => {
  ...
  if (!id) return
  await employeeStore.getEmployee(id)
})
  ...
  (val) => {
    ...
    resetForm({
      values: {
        ...
        EmployeeName: (val.employeeName ?? '').toString(),
        EmployeeEmail: (val.employeeEmail ?? '').toString(),
        PositionName: (val.positionName ?? '').toString(),
        CompanyCode: ((val.companyCode ?? '') as string).toUpperCase(),
        CostCenter: (val.costCenter ?? '').toString(),
