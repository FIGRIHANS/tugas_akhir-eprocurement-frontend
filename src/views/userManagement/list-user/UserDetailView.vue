        ...
        <div class="card">
          ...
          <div class="card-body">
            <div class="card-group flex justify-between">
              ...
              <p class="text-sm font-medium text-slate-700">{{ userDetail?.profile?.profileId }}</p>
            </div>
            <div class="card-group flex justify-between">
              <p class="text-sm font-medium text-slate-500">Full Name</p>
              <p class="text-sm font-medium text-slate-700">
                {{ userDetail?.profile?.employeeName }}
              </p>
            </div>
            <div class="card-group flex justify-between">
              <p class="text-sm font-medium text-slate-500">Profile</p>
              <p class="text-sm font-medium text-slate-700">
                {{ userDetail?.profile?.positionName }}
              </p>
            </div>
            <div class="card-group flex justify-between">
              <p class="text-sm font-medium text-slate-500">Email</p>
              <p class="text-sm font-medium text-slate-700">
                {{ userDetail?.profile?.employeeEmail }}
              </p>
            </div>
            <div class="card-group flex justify-between">
...
const formattedLastLogin = computed(() => {
  const raw = userDetail.value?.profile?.lastLoginDate
  if (!raw) return '-'
  const m = moment(raw)
