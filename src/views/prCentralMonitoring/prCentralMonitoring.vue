<template>
  <div>
    ...
    <section v-if="isFiltering">
      <div class="flex flex-row mt-[24px] align-center p-[8px] gap-[24px]">
        ...
        <div
          v-if="filter.purchReq != ''"
          class="inline-flex items-center gap-2 rounded-md border border-gray-300 bg-white px-3 py-1 text-sm text-gray-700 shadow-sm"
        >
          <span>Purch Req</span>
          <span class="text-black">{{ filter.purchReq }}</span>
          <button
            class="text-gray-500 hover:text-gray-700 focus:outline-none"
            @click="((filter.purchReq = ''), (isFiltering = false))"
          >
            <svg
        ...
        >
          ...
          <button
            class="text-gray-500 hover:text-gray-700 focus:outline-none"
            @click="((filter.prScenario = ''), (isFiltering = false))"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="h-4 w-4"
        ...
        >
          ...
          <button
            class="text-gray-500 hover:text-gray-700 focus:outline-none"
            @click="((filter.tenderSattus = ''), (isFiltering = false))"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="h-4 w-4"
        ...
        >
          ...
          <button
            class="text-gray-500 hover:text-gray-700 focus:outline-none"
            @click="((filter.materialNo = ''), (isFiltering = false))"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="h-4 w-4"
        ...
        >
          ...
          <button
            class="text-gray-500 hover:text-gray-700 focus:outline-none"
            @click="((filter.deliveryDate = ''), (isFiltering = false))"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="h-4 w-4"
        ...
        >
          ...
          <button
            class="text-gray-500 hover:text-gray-700 focus:outline-none"
            @click="((filter.materialGroup = ''), (isFiltering = false))"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="h-4 w-4"
    ...
    >
      <div class="flex flex-row gap-[24px] justify-between w-full">
        ...
        <div class="flex flex-row gap-[8px]">
          <UiButton outline variant="primary" @click="isModalFilterOpen = true"
            ><i class="ki-duotone ki-filter"></i> Filter</UiButton
          >
          <UiButton outline variant="primary"
            ><i class="ki-duotone ki-printer"></i>Export Data</UiButton
          >
          <UiButton outline variant="primary"
            ><i class="ki-duotone ki-arrows-circle"></i>SAP Sync</UiButton
          >
        </div>
      </div>
      <div class="scrollable-x-auto">
        <table class="table align-middle">
          ...
          <tbody>
            ...
            <tr v-else v-for="item in prData" :key="item.id" class="text-nowrap">
              <td>
                <input
  ...
  >
    <div class="space-y-5 mt-[24px]">
      <UiSelect
        label="Purchasing Req"
        :options="lookupPurchList"
        text-key="text"
        value-key="key"
        v-model="filter.purchReq"
      />
      <UiSelect
        label="Pr Scenario"
        :options="lookupPrScenario"
        text-key="text"
        value-key="key"
        v-model="filter.prScenario"
      />
      <UiSelect
        label="Tender Status"
        :options="lookupTenderStatus"
        text-key="text"
        value-key="key"
        v-model="filter.tenderSattus"
      />
      <UiSelect
        label="Material No"
        :options="lookupMaterialNo"
        text-key="text"
        value-key="key"
        v-model="filter.materialNo"
      />
      <UiInput label="Delivery Date" type="date" v-model="filter.deliveryDate" />
      <UiSelect
        label="Material Group"
        :options="lookupMaterialGroup"
        text-key="text"
        value-key="key"
    ...
    <div class="flex flex-row gap-[8px] justify-center mt-[24px]">
      <UiButton variant="primary" outline @click="resetFilter()"
        ><i class="ki-duotone ki-arrows-circle"></i>Reset</UiButton
      >
      <UiButton variant="primary" @click="getFilter()"
        ><i class="ki-filled ki-check-circle"></i>Terapkan</UiButton
      >
    </div>
  </UiModal>
</template>
...
import ChartCard from '@/components/ui/atoms/chartCard/chartCard.vue'
