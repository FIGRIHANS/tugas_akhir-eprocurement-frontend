<template>
  <div>
    <Breadcrumb title="Contract List" :routes="routes" />
    ...
    <div class="border rounded-md p-[24px] mt-[24px]">
      ...
      <div v-if="checkFilterForm" class="mt-[24px] flex align-items-center gap-[16px]">
        ...
        <span v-if="filterForm.contractRequestId" class="badge badge-outline">
          Contract Request ID
          <span class="text-gray-800 ml-[4px]">{{ filterForm.contractRequestId }}</span>
          <i
            class="ki-filled ki-cross text-gray-800 ml-[10px] cursor-pointer"
            @click="
              () => {
                filterForm.contractRequestId = ''
                pageNo = 1
              }
            "
          ></i>
          ...
          <thead>
            <tr>
              <th
                ...
                :class="{
                  'report__field-base--long': item === 'Contract Status' || item === 'PIC Tender',
                }"
              >
                {{ item }}
              </th>
                    ...
                    <div class="menu menu-default flex flex-col w-fit">
                      <div class="menu-item">
                        <div class="menu-link" href="#">
                          ...
                          <span class="menu-title"> Created </span>
                        </div>
                      </div>
                      <div class="menu-item">
                      ...
                      <div class="menu-item">
                        <div class="menu-link" href="#" @click="goToEvaluation">
                          ...
                          <span class="menu-title"> Vendor Feedback </span>
                        </div>
                      </div>
                      <div class="menu-item">
                        <div class="menu-link" href="#" @click="goToNegotiation">
                          ...
                          <span class="menu-title"> Finalization </span>
                        </div>
                      </div>
                      <div class="menu-item">
...
import CardGraph from './ContractList/CardGraph.vue'
import LPagination from '@/components/pagination/LPagination.vue'
import FilterList from './ContractList/FilterList.vue'
import { useTenderStore } from '@/stores/views/tender/tender'
import type { routeTypes } from '@/core/type/components/breadcrumb'
import type { CardGraphTypes, ReportListTypes, filterListTypes } from './types/contractList'
...
/** BREADCRUMB */
const routes = ref<routeTypes[]>([{ name: 'Contract Report List', to: '/contract' }])
...
const checkFilterForm = computed(() => {
  return Boolean(
    filterForm.value.contractRequestId ||
...
const setFilter = (data: filterListTypes) => {
  filterForm.value.contractRequestId = data.contractRequestId
...
const filteredReports = computed(() => {
  ...
  return dummyReport.value.filter((row) => {
    ...
    if (filterForm.value.contractRequestId && norm(row.id) !== norm(filterForm.value.contractRequestId))
      return false
