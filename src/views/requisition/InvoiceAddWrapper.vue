<template>
  <div>
    <Breadcrumb title="Add Requisition" :routes="routes" />
    ...
    <div>
      ...
      >
        ...
        <div class="flex-1 flex gap-[8px] justify-end">
          <button
            ...
            :disabled="isSubmit || tabNow !== 'information'"
            @click="checkBudget"
          >
            Budget Checking
      ...
      <div
        v-else-if="
          (form.status === 0 || form.status === -1 || form.status === 5) && !checkInvoiceView()
        "
      ...
      <div v-else class="flex justify-end items-center mt-[24px]">
        <button
          v-if="tabNow !== 'preview' || checkInvoiceView()"
...
const tabNow = ref<string>('information')
...
const initialData = {
  invoiceUId: '',
  invoiceType: '2',
  invoiceTypeName: 'Project Requisition',
  vendorId: '',
  vendorName: '',
  npwp: '',
  address: '',
  ...
  invoiceNo: '',
  companyCode: 'GNLA',
  companyName: 'PT. Erha Medical',
  invoiceNoVendor: '10',
  invoiceDate: '2025-09-28T01:05:00.000Z',
  taxNumber: '',
  invoiceDp: '2',
  amountInvoice: '',
  taxNoInvoice: '12312412312',
  remainingDpAmount: '',
  dpAmountDeduction: '',
  currency: 'IDR',
  ...
  department: '',
  purchasingGroup: '2',
  RequisitionCode: 'PROD2',
}

onMounted(() => {
  if (route.name == 'requisitionEdit') {
    if (form) {
