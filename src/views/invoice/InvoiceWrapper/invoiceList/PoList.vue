        ...
        <tbody>
          ...
          <template v-for="(parent, index) in list" :key="index">
            <tr>
              ...
              <td>
                <div v-if="parent.whtReconciliationStatus" class="flex items-center gap-1">
                  <span 
                    class="badge badge-dot size-2" 
                    :class="parent.whtReconciliationStatus === 'NORMAL-Done' ? 'badge-success' : 'badge-warning'"
                  ></span>
                  <span class="text-xs font-semibold">{{ parent.whtReconciliationStatus }}</span>
                </div>
