<template>
  ...
  <div class="card">
    ...
    <div class="card-body">
          ...
          <div class="p-4">
            <div class="flex items-center gap-3">
              ...
              <div class="flex-1 max-w-[400px]">
                <UiSelect
                  v-model="wfHeader.organizationStructure"
                  placeholder="Select"
                  row
                  valueKey="code"
                  textKey="name"
                  required
                  :options="organizationStructureList"
                  :disabled="isReadOnly"
                  class="flex-1 max-w-[400px]"
                />
              </div>
            ...
            <div class="flex items-center gap-3">
              ...
              <div class="flex-1 max-w-[400px]">
                <div class="relative flex items-center border border-gray-300 rounded-md bg-white">
                  ...
                  <input
                    type="text"
                    class="flex-1 h-10 px-3 text-sm border-0 bg-white focus:outline-none focus:ring-0"
                    placeholder="Bracket Code"
                    :value="selectedBracket?.bracketCode ?? ''"
                    readonly
                  />
                </div>
              </div>
      ...
      <div v-if="!isReadOnly" class="flex justify-end mt-6 pr-4">
        <UiButton
          class="bg-primary hover:bg-teal-600 text-white font-medium rounded h-12 px-5 shadow-sm flex items-center gap-2"
  ...
  <div class="mt-6 flex justify-between items-center px-2">
    <UiButton
      class="bg-white border border-primary-clarity text-primary hover:bg-primary hover:text-white h-10 px-3 flex items-center gap-2 shadow-sm"
      @click="goBack"
    >
      <svg
        class="w-4 h-4"
    ...
    <UiButton
      v-if="!isReadOnly"
      class="bg-primary hover:bg-teal-600 text-white font-semibold h-12 px-5 shadow-lg flex items-center gap-2"
      @click="saveWorkflow"
    >
      <span>Submit</span>
      <svg
  ...
  <!-- Modals -->
  <AddBracketModal
    v-if="showBracketModal"
    @close="showBracketModal = false"
    @select="handleBracketSelect"
  />
  ...
  <UiModal v-model="showAddWorkflowModal" size="md" hide-header hide-close>
    <div class="relative w-full max-w-2xl mx-auto">
      ...
      <div class="flex items-center justify-end gap-2 px-4 py-3 border-t">
        <UiButton class="" @click="closeAddWorkflowModal"> Cancel </UiButton>
        <UiButton class="" @click="generateWorkflowSteps"> Generate Step </UiButton>
      </div>
    </div>
  </UiModal>
  ...
  <UiModal v-model="showEditRowModal" size="lg" title="Edit Workflow Step">
    <div v-if="editRowDraft" class="grid grid-cols-2 gap-4">
      ...
      <div>
        ...
        <UiInput
          v-model="editRowDraft.profileId"
          type="text"
          class="h-9 w-full brder px-2 text-sm"
        />
      </div>
      <div>
      ...
      <div>
        ...
        <UiInput
          v-model="editRowDraft.approverGroupId"
          type="text"
          class="h-9 w-full px-2 text-sm"
        />
      </div>
      <div>
        <label class="text-xs text-gray-500">Notification Group ID</label>
        <UiInput
          v-model="editRowDraft.notificationGroupId"
          type="text"
          class="h-9 w-full px-2 text-sm"
        />
      </div>
    ...
    <div class="flex items-center justify-end gap-2 border-t mt-4 pt-3">
      <UiButton class="btn btn-outline btn-light" @click="closeEditRowModal"> Cancel </UiButton>
      <UiButton
        class="px-3 py-1 text-sm text-white bg-primary hover:bg-teal-600"
        @click="saveEditRow"
      >
        Save
      </UiButton>
...
import type {
  ...
  AuthForm,
  ProfileForm,
} from '@/views/workflowConfiguration/types/Workflow'
...
onMounted(async () => {
  ...
  if (wfCode) {
    ...
    if (resolvedWorkflow) {
      ...
      const profileRows =
        resolvedWorkflow.workflow?.map((row) => ({
          step: row.step,
          category: row.category || '-',
          bracketAmount: row.bracketAmount,
          stepName: row.stepName || '-',
